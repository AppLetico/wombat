...
const timezone = process.env.STANDUP_TIMEZONE || "UTC";
...
async function main() {
  ...
  const resp = await fetch(`${config.backendUrl}/api/mission-control/standup`, {
    method: "POST",
    headers: {
  ...
  if (!resp.ok) {
    throw new Error(`standup failed ${resp.status}`);
  }
